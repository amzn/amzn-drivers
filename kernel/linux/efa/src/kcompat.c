...
static int UVERBS_HANDLER(UVERBS_METHOD_COMP_CNTR_CREATE)(struct uverbs_attr_bundle *attrs)
{
	...
	ret = uverbs_copy_to(attrs, UVERBS_ATTR_CREATE_COMP_CNTR_RESP_COUNT_MAX_VALUE,
			     &cc->ibcc.comp_count_max_value,
			     sizeof(cc->ibcc.comp_count_max_value));
	if (ret)
		goto err_copy;

	ret = uverbs_copy_to(attrs, UVERBS_ATTR_CREATE_COMP_CNTR_RESP_ERR_COUNT_MAX_VALUE,
			     &cc->ibcc.err_count_max_value,
			     sizeof(cc->ibcc.err_count_max_value));
	if (ret)
		goto err_copy;
...
err_copy:
#ifndef HAVE_UVERBS_FINALIZE_UOBJ_CREATE
	efa_destroy_comp_cntr(&cc->ibcc);
	kfree(cc);
	uobj->object = NULL;
#endif
