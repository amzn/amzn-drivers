...
function(run_conftests)
  set(cmds_file "${CMAKE_CURRENT_BINARY_DIR}/conftest_cmds.txt")
  file(WRITE ${cmds_file} "")
  foreach(cmd ${conftest_cmds})
    file(APPEND ${cmds_file} "${cmd}\n")
  endforeach()
  execute_process(
    COMMAND ${CMAKE_SOURCE_DIR}/config/run_parallel.sh ${cmds_file}
    WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR})
  file(REMOVE ${cmds_file})
endfunction()

set(conftest_cmds "" CACHE INTERNAL "")
...
function(add_dev_op_compile_conftest fp_name prologue fn success_def fail_def)
...
add_compile_conftest("#include <rdma/ib_umem.h>" "struct ib_umem_chunk chunk;" "" "HAVE_UMEM_SCATTERLIST_IF")

add_compile_conftest("" "struct ib_cq_init_attr attr;" HAVE_CREATE_CQ_ATTR "")

add_compile_conftest("" "struct rdma_ah_attr attr;" HAVE_CREATE_AH_RDMA_ATTR "")

add_dev_op_compile_conftest(post_send ""
  "
...
add_compile_conftest("static int port_callback(struct ib_device *ibdev, u8 n, struct kobject *kobj) { return 0; }"
  "
char name[2];
...
add_compile_conftest("" "ib_modify_qp_is_ok(0, 0, 0, 0);" HAVE_IB_MODIFY_QP_IS_OK_FOUR_PARAMS "")
...
add_compile_conftest("" "struct ib_device_ops ops;" HAVE_IB_DEV_OPS "")

add_dev_op_compile_conftest(create_ah ""
...
add_dev_op_compile_conftest(dealloc_pd ""
  "void efa_dealloc_pd(struct ib_pd *ibpd, struct ib_udata *udata) {}"
  HAVE_DEALLOC_PD_UDATA "")

add_dev_op_compile_conftest(dereg_mr ""
  "int efa_dereg_mr(struct ib_mr *ibmr, struct ib_udata *udata) { return 0; }"
  HAVE_DEREG_MR_UDATA "")

add_dev_op_compile_conftest(destroy_qp ""
...
add_compile_conftest("#include <rdma/ib_umem.h>" "ib_umem_find_best_pgsz(NULL, 0, 0);"
  HAVE_IB_UMEM_FIND_SINGLE_PG_SIZE "")

add_compile_conftest("" "enum rdma_driver_id driver_id = RDMA_DRIVER_EFA;"
  HAVE_UPSTREAM_EFA "")
...
add_dev_op_compile_conftest(destroy_cq ""
...
add_compile_conftest("" "int a = IB_QPT_DRIVER;" HAVE_IB_QPT_DRIVER "")
...
add_compile_conftest("#include <linux/pci_ids.h>" "int a = PCI_VENDOR_ID_AMAZON;"
  HAVE_PCI_VENDOR_ID_AMAZON "")

add_compile_conftest("#include <rdma/ib_umem.h>" "ib_umem_get(NULL, 0, 0, 0);"
...
add_compile_conftest("" "int a = RDMA_NODE_UNSPECIFIED;" HAVE_RDMA_NODE_UNSPECIFIED "")
...
add_dev_op_compile_conftest(alloc_mr ""
...
add_dev_op_compile_conftest(destroy_ah ""
...
add_dev_op_compile_conftest(create_user_ah ""
...
add_dev_op_compile_conftest(query_port ""
  "int efa_query_port(struct ib_device *ibdev, u32 port, struct ib_port_attr *props) { return 0; }"
  HAVE_U32_PORT "")

add_dev_op_compile_conftest(alloc_hw_port_stats ""
...
add_compile_conftest("#include <linux/xarray.h>" "xa_load(NULL, 0);" HAVE_XARRAY "")
...
add_compile_conftest("" "struct rdma_stat_desc desc;" HAVE_STAT_DESC_STRUCT "")
...
add_dev_op_compile_conftest(reg_user_mr_dmabuf ""
  "
...
add_compile_conftest("#include <rdma/ib_umem.h>" "ib_umem_dmabuf_get_pinned(NULL, 0, 0, 0, 0);" HAVE_IB_UMEM_DMABUF_PINNED "")

add_compile_conftest("#include <linux/module.h>" "MODULE_IMPORT_NS(TEST);" HAVE_MODULE_IMPORT_NS "")
